...
class Workload:
    ...
    def _create_commands_from_options(self) -> None:
        ...
        for unique_options in all_configs(self._all_options):  # type: ignore[no-untyped-call]
            ...
            for target, iodepth in iodepth_per_target.items():
                # command_list: list[Command] = []
                unique_options["iodepth"] = f"{iodepth}"
                # for target_number in [target for target, iod in iodepth_per_target.items() if iod == iodepth]:
                unique_options["target_number"] = f"{target}"
